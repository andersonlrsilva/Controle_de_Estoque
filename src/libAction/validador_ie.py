# CODIGO VALIDADOR DE IE FOI ESCRITO POR:
# Fabio Fila
# https://github.com/filati
"""
...
def validaIeRg(uf, dados, tipo):
    if tipo == 'Juridico':
        result = validar(uf=uf, numero=dados)
        if result is False:
            return False
...
def pernambuco(ie):
    def core(n):
        if len(n) == 9:                   # formato atual (eFisco): 7 base + 2 DV
            ...
            d2 = _dv_mod11(_soma_pesos(
                base, [9, 8, 7, 6, 5, 4, 3, 2]) + d1 * 2)
            return n[7:] == f"{d1}{d2}"
...
def validar(uf, numero):
    ...
    if uf not in VALIDADORES:
        raise ValueError(
            f"UF desconhecida: {uf!r}. Use uma de {sorted(VALIDADORES)}")
...
def testar(verbose=True):
    ...
    def checar(uf, num, esperado):
        ...
        if verbose:
            ...
            print(
                f"  [{marca}] {uf} {num!r:24} valido={ok} (esperado {esperado})")
    ...
    for uf, num in EXEMPLOS_VALIDOS.items():
        d = _digitos(num)
        errado = num.replace(
            d[-1], str((int(d[-1]) + 1) % 10), 1) if d else num
